...
interface MultiProjectStore {
  ...
  /** Restaure un brouillon (anti-crash) dans le store et ouvre le projet */
  restoreDraft: (project: ProjectData) => void;
}
  ...
  (set, get) => ({
    ...
    closeProject: async () => {
      ...
      if (currentProjectId) {
        ...
        try {
          localStorage.removeItem("cirad-draft-backup");
        } catch {}
      } else {
    ...
    restoreDraft: (project) => {
      const proj = migrateToMultiLot(project);
      set((state) => ({
        projects: { ...state.projects, [proj.id]: proj },
